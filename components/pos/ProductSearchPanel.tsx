...
import { Search, Package, ShoppingCart, ScanLine, CheckCircle2 } from "lucide-react";
...
export function ProductSearchPanel({ products, currency }: Props) {
  ...
  const [scanResult, setScanResult] = useState<{ name: string; found: boolean } | null>(null);
  ...
  // Handles USB/Bluetooth scanner (fast keystrokes + Enter)
  ...
  function handleCameraScan(barcode: string) {
    ...
    if (match) {
      addProduct(match);
      setScanResult({ name: match.name, found: true });
    } else {
      setScanResult({ name: barcode, found: false });
    }
        ...
        <div className="p-3 bg-white border-b border-slate-200">
          <div className="relative flex gap-2">
            ...
            <button
              ...
              className="flex-shrink-0 flex items-center justify-center w-10 h-10 bg-green-600 hover:bg-green-700 text-white rounded-lg transition shadow-sm"
            >
          ...
          {scanResult && (
            <div className={cn(
              "flex items-center gap-2 mt-2 px-3 py-2 rounded-lg text-sm font-medium transition-all",
              scanResult.found
              ...
              {scanResult.found ? (
                <>
                  ...
                  <span><strong>{scanResult.name}</strong> added to cart</span>
                </>
              ) : (
                <>
                  <Package className="w-4 h-4 flex-shrink-0" />
                  <span>No product found for <strong>{scanResult.name}</strong></span>
                </>
              )}
            </div>
          )}
