...
export function generateStatementHTML(
  ...
  const rows = entries.map((e) => `
    <tr>
      ...
      <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;font-size:13px;text-align:right;color:${e.credit > 0 ? "#1a9c38" : "#9ca3af"}">${e.credit > 0 ? fmt(e.credit) : "—"}</td>
      <td style="padding:10px 12px;border-bottom:1px solid #f1f5f9;font-size:13px;text-align:right;font-weight:600;color:${e.balance > 0 ? "#dc2626" : "#1a9c38"}">${fmt(Math.abs(e.balance))} ${e.balance > 0 ? "DR" : "CR"}</td>
    </tr>
  `).join("");
...
<body style="padding:40px;max-width:800px;margin:0 auto">
  ...
  <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;margin-bottom:28px">
    <div style="border:1px solid #e2e8f0;border-radius:10px;padding:16px">
      ...
      <p style="font-size:18px;font-weight:800;margin-top:4px;color:${openingBalance > 0 ? "#dc2626" : "#1a9c38"}">${fmt(Math.abs(openingBalance))}</p>
    </div>
    <div style="border:1px solid #e2e8f0;border-radius:10px;padding:16px">
    ...
    <div style="border:1px solid #e2e8f0;border-radius:10px;padding:16px">
      ...
      <p style="font-size:18px;font-weight:800;margin-top:4px;color:${closingBalance > 0 ? "#dc2626" : "#1a9c38"}">${fmt(Math.abs(closingBalance))}</p>
    </div>
  </div>
  ...
  <div style="margin-top:32px;border-top:1px solid #e2e8f0;padding-top:16px;display:flex;justify-content:space-between">
    ...
    ${closingBalance > 0
      ...
      : `<p style="font-size:13px;font-weight:700;color:#1a9c38">Account clear</p>`}
  </div>
</body>
</html>`;
}
